...
CREATE PROCEDURE `fetch_food_transactions`(IN `osca_id` varchar(20))
BEGIN
  SELECT
      `f`.trans_date,
      `f`.company_name,
...
CREATE PROCEDURE `fetch_password`(IN `osca_id` varchar(20))
BEGIN
  SELECT
    `mg`.osca_id,
    `mg`.`password`,
...
BEGIN
  SELECT
    `p`.trans_date, `p`.company_name, `p`.branch, `p`.business_type,
    concat(`p`.generic_name, '\n' , `p`.brand, '\n',
    `p`.dose, '\n', `p`.unit, '\n', `p`.quantity, '\n', `p`.unit_price) AS `desc`, 
    `p`.vat_exempt_price, `p`.discount_price, `p`.payable_price
  FROM
...
BEGIN
  SELECT
    `t`.trans_date, `t`.company_name, `t`.branch, `t`.business_type, `t`.`desc`, `t`.vat_exempt_price, `t`.discount_price, `t`.payable_price
  FROM
...
CREATE DEFINER=`dbosca`@`localhost` PROCEDURE `validate_login`(IN `osca_id` varchar(120), IN `password` VARCHAR(120))
BEGIN
...
DROP PROCEDURE IF EXISTS `fetch_food_transactions`;;
CREATE PROCEDURE `fetch_food_transactions` (IN `osca_id_` INT)  BEGIN
...
DROP PROCEDURE IF EXISTS `fetch_password`;;
CREATE PROCEDURE `fetch_password` (IN `osca_id_` INT)
BEGIN
  SELECT
...
CREATE TABLE `city_code` (
  ...
  `citymunDesc` text CHARACTER SET utf8,
